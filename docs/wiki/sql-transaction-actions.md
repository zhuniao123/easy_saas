...
- **write**：允许以 `SELECT`（少见）、`INSERT`、`UPDATE`、`DELETE` 开头（大小写不敏感，忽略前导空白/注释不支持——先要求干净 SQL）。  
- **assert**：必须是 `SELECT`；结果集至少 1 行，且若存在 `ok` 列则须为 true/t/1；失败则整事务 rollback。
